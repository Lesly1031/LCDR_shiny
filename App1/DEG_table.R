...
limma_UI<-function(headings,output_info,voom_dt,download_info,download_info2,group_info,phenotype,deg_meta,study_type){
  if(grepl("metabolomics",study_type)){
    column(6,
           panel(
             status="info",heading=paste0("Study: ",headings),
             dropdown(
               radioButtons(deg_meta,"Only include identified metabolites?",c("Yes","No"),"Yes"),
  ...
  } else if(grepl("lipidomics",study_type)){
    column(6,
           panel(
             status="info",heading=paste0("Study: ",headings),
             dropdown(
               radioButtons(deg_meta,"Collapse class for lipidomics?",c("Yes","No"),"Yes"),
  ...
  }else if (grepl("RNAseq",study_type)){ ############## check on server
    column(6,
           panel(
             status="info",heading=paste0("Study: ",headings),
             dropdown(
               radioButtons(deg_meta,"",c(paste0("This is ",study_type," study")),paste0("This is ",study_type," study")),
               selectizeInput(group_info, label="Select group annotation", 
                              choices =limma_choices(phenotype)),
               downloadButton(download_info, "Download DEG table"),
               downloadButton(download_info2, "Download voom data"),
             ...
             tabsetPanel(
               ...
               tabPanel("Normalized data (Voom TMM)",
                        conditionalPanel( 
                          condition="input.update_DEG==0",
                          br(),
                          p("Please click 'Run' button to generate results")
                        ...
                        conditionalPanel( 
                          condition="input.update_DEG>0",
                          br(),
                          withLoader( DTOutput(voom_dt),type="html",loader="loader5")
...
limma_voom = function(input,output,session,folder_direct,data,data_class,group_info){
  ...
  data.frame(rnaseq_voomed$E)
}

limma_server<-function(input,output,session,folder_direct, data_identify,data_lip,data,data_class,group_info,study_type,deg_meta){
  ...
  if(grepl("metabolomics",folder_direct)|grepl("proteomics",folder_direct)|grepl("lipidomics",folder_direct)){ ########### metabolomics ##################
    ...
    res=res[order(res$padj,res$log2FoldChange,decreasing = F),]
    for(i in 1:4){
    ...
    fit1 = lmFit(rnaseq_voomed, rnaseq_voomed$design)
    ...
    res=res[order(res$padj,res$`log odds of differential expression`,decreasing = F),]
