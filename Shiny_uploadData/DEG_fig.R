...
deg_figServer<-function(input,output,session,diff_result,fill_ma,title){
  ...
  if(input[["plot_type_MA"]] =="MA plot"){
    ...
    if(is.null(input[[fill_ma]])){
      ...
      res$sel=ifelse(res$gene_name%in%as.character(make.names(input[[fill_ma]])),"Selected","Not selected")
      res$group=factor(res$group,levels=c("Up","Down","NS"))
